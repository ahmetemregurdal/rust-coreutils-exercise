...
fn parse_backslash(a: String) -> String {
    let mut new_message: String = "".to_string();
    let original_text_vec: Vec<char> = a.chars().collect();
    let mut skip = false;
...
fn main() {
    ...
    if !cli.enable_backslash {
        ...
        if cli.no_newline {
            print!("{}", parse_backslash(cli.message.unwrap()));
        } else {
            println!("{}", parse_backslash(cli.message.unwrap()));
        }
    }
}
